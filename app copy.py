    ...
    def dax_sql(text):
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Convert the respective PowerBI measure to MySQL statement.{text} Provide me only the sql measures Query as output."
                }
            ]
        )
...
def process_lookml(file_content):
    ...
    for line in lines:
        if len(line.strip()) != 0:
            ...
            if  view_count > view_conter:
                view_conter = view_conter+1
                for types_repl in types:
                    types_repls.append(types_repl)
                types = []
    ...
    for m in model:
        ...
        er1.append(mermaid_er)
    return model, looker_measure.to_dict(orient='records'), er, explore, er1
...
def process_file():
    ...
    if file:
        ...
        model, looker_measure, er,explore,er1 = process_lookml(file_content)
        return jsonify({'explore_name':explore,'model': model, 'looker_measure': looker_measure, 'er': er, 'er1':er1})
